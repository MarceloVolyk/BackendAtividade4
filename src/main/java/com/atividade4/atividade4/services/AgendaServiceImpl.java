...
public class AgendaServiceImpl {
    ...
    @Transactional
    public AgendaResponseDTO atualizarAgenda(Long id, AgendaDTO agendaDTO) {
        ...
        // Verificar se o professor já está agendado para o período (excluindo a agenda atual)
        boolean professorIndisponivel = false;
        if (!agenda.getProfessor().getId().equals(professor.getId())) {
            professorIndisponivel = agendaRepository.existsAgendamentoConflitante(
                    professor.getId(), agendaDTO.getDataInicio(), agendaDTO.getDataFim());
        }
    ...
    public void excluirAgenda(Long id) {
        if (!agendaRepository.existsById(id)) {
            throw new RegraNegocioException("Agenda não encontrada com ID: " + id);
        }
